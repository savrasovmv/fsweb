...
export const Settings = () => {
    ...
    useEffect(() => {
        ...
        APIClient.v1.get(DOMAIN_TABLE, {})
            .then((resolve) => {
                ...
                APIClient.v1.get(CONTEXT_TABLE, {})
                .then((resolve) => {
                    if (resolve.length > 0) {
                        console.log('context', resolve)
                        setContext({id:resolve[0].id, name: resolve[0].name})
    ...
    const handleSubmitClick = (data) => {
        ...
        if (data.domain) {
            ...
            APIClient.v1.post(DOMAIN_TABLE, {}, { isCreate: isCreateDomain, data: dataDomain })
            .then((resolve) => {
        ...
        if (data.context) {
            ...
            APIClient.v1.post(CONTEXT_TABLE, {}, { isCreate: isCreateContext, data: dataContext })
            .then((resolve) => {
                console.log('result', resolve)
                //history.goBack()
            })
            ...
            <form noValidate autoComplete="off" onSubmit={handleSubmit(handleSubmitClick)}>
                {open ? (
                    <Box width="50%" m={1}>
                        ...
                        <FormField name="context" errors={errors} register={register}/>
